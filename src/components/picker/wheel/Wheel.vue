<template>
  <div :class="{s_disabled: disabled}"
       class="cpw_container col-shrink"
       ref="color-wheel"
       id="color-wheel"
...
export default {
  ...
  mounted () {
    if (this.hasCamelCaseColorChangeListener) {
      this.$log.warn(`Using the colorChange event is deprecated since version 0.4.0. It will be deleted in version 1.0.0. 'v-model' or the kebab-case variant 'color-change' should be used.`)
    }
    this.initWidget()
    this.setColor(this.bindColor || this.startColor || DEFAULT_START_COLOR, true)
  },
  data () {
    return {
      debug: false,
  ...
  methods: {
    ...
    drawMarkers () {
      ...
      const c1 = this.invert ? '#a0aec0' : '#2d3748'
      const c2 = this.invert ? '#2d3748' : '#a0aec0'
      const circles = [
        { x: x1, y: y1, r, c: '#2d3748', lw: lw + 1 },
        { x: x1, y: y1, r: this.markerSize, c: '#a0aec0', lw },
    ...
    updateDisplay (noEmit) {
      ...
      if (!noEmit) {
        ...
        this.setComponentState({ state: 'rgb', value: this.color })
      }
    },
    widgetCoords (event) {
      return {
